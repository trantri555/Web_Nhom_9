                        ...
                        <div class="container">
                            ...
                            <div class="collapse navbar-collapse" id="navbarNav">
                                ...
                                <a href="${pageContext.request.contextPath}/login"
                                    class="btn btn-warning rounded-pill ms-lg-3 fw-semibold shadow-sm">Đăng Nhập</a>
                                <a href="${pageContext.request.contextPath}/order"
                                    class="btn btn-warning rounded-pill ms-lg-3 fw-semibold shadow-sm">
                                    ...
                                    <div class="filter-body">
                                        <form action="products" method="get">
                                            <%-- Lọc theo Giá --%>
                                                <div class="mb-4">
                                                    <label
                                                        class="form-label fw-bold text-dark small text-uppercase">Khoảng
                                                        giá</label>
                                                    <div class="input-group input-group-sm mb-2">
                                                        <span class="input-group-text bg-white">₫</span>
                                                        <input type="number" name="minPrice" class="form-control"
                                                            placeholder="Từ" value="${currentMinPrice}">
                                                    </div>
                                                    <div class="input-group input-group-sm">
                                                        <span class="input-group-text bg-white">₫</span>
                                                        <input type="number" name="maxPrice" class="form-control"
                                                            placeholder="Đến" value="${currentMaxPrice}">
                                                    </div>
                                                </div>
                                                ...
                                                <%-- Lọc theo Thể Tích --%>
                                                    <div class="mb-4">
                                                        <label
                                                            class="form-label fw-bold text-dark small text-uppercase">Thể
                                                            tích</label>
                                                        <select name="volume" class="form-select form-select-sm">
                                                            <option value="">Tất cả</option>
                                                            <c:forEach items="${volumeList}" var="vol">
                                                                <option value="${vol}" ${currentVolume==vol ? 'selected'
                                                                    : '' }>${vol} ml</option>
                                                            </c:forEach>
                                                        </select>
                                                    </div>
                                                    ...
                                                    <%-- Lọc theo Nhà cung cấp --%>
                                                        <div class="mb-4">
                                                            <label
                                                                class="form-label fw-bold text-dark small text-uppercase">Nhà
                                                                cung cấp</label>
                                                            <select name="supplier" class="form-select form-select-sm">
                                                                <option value="">Tất cả</option>
                                                                <c:forEach items="${supplierList}" var="sup">
                                                                    <option value="${sup}" ${currentSupplier==sup
                                                                        ? 'selected' : '' }>${sup}</option>
                                                                </c:forEach>
                                                            </select>
                                                        </div>
                                                        ...
                                                        <%-- Sắp xếp --%>
                                                            <div class="mb-4">
                                                                <label
                                                                    class="form-label fw-bold text-dark small text-uppercase">Sắp
                                                                    xếp</label>
                                                                <select name="sort" class="form-select form-select-sm">
                                                                    <option value="">Mặc định</option>
                                                                    <option value="priceAsc" ${currentSort=='priceAsc'
                                                                        ? 'selected' : '' }>Giá tăng dần</option>
                                                                    <option value="priceDesc" ${currentSort=='priceDesc'
                                                                        ? 'selected' : '' }>Giá giảm dần</option>
                                                                    <option value="nameAsc" ${currentSort=='nameAsc'
                                                                        ? 'selected' : '' }>Tên A-Z</option>
                                                                </select>
                                                            </div>

                                                            <button type="submit"
                                                                class="btn btn-primary-custom w-100 fw-bold py-2 rounded-pill shadow-sm">
                                                                <i class="bi bi-check2-circle me-1"></i> Áp Dụng
                                                            </button>
                                                            <div class="text-center mt-3">
                                                                <a href="products"
                                                                    class="text-secondary text-decoration-none small hover-link">
